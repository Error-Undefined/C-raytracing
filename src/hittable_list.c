...
struct hittable_list* init_hittable_list(void* object_data, hittable_type obj_type)
{
  ...
  struct hittable_list_node* list_node = malloc(sizeof(struct hittable_list_node));
  if(list_node == NULL)
  {
    free(list);
    return NULL;
